...
unordered_map<int, int> findMinimumTime(int buildings, const vector<vector<int> >& roads, int start) {
    ...
    // an adjacency list from the list of roads, to keep track of the roads between buildings 
    ...
    // Perform BFS
    queue<int> q;
    q.push(start);// begin the search, while que is not empty 
    while (!q.empty()) {
        int currentBuilding = q.front();// currut building is at the top of the que
        q.pop();// move to next
